...
interface Pregunta {
  ...
  respuestas?: Array<{ url_archivo?: string }>;
}
...
export default function VerCapacitacion() {
  ...
  const loadCapacitacion = async () => {
    try {
      ...
      const data: any = await CapListService.getCapacitacionById(id);
    ...
    return (
      <div className={styles.container}>
        <div className={styles.errorBox}>
          ...
          <button className={styles.btnBack} onClick={() => navigate("/capacitaciones")}>
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        ...
        <div className={styles.imageContainer}>
          {capacitacion.imagen ? (
            <img src={normalizeDataUrl(capacitacion.imagen, 'image')} alt={capacitacion.titulo} className={styles.headerImage} />
          ) : (
                    ...
                    modulo.lecciones.map((leccion, leccionIndex) => (
                      <div key={leccionIndex} className={styles.leccionItem}>
                        ...
                        <div className={styles.leccionInfo}>
                          ...
                          {/* Preview thumbnail */}
                          {leccion.tipo_leccion === 'formulario' && leccion.preguntas && leccion.preguntas[0]?.url_multimedia && (
                            <div className={styles.leccionThumbnail}>
                              <img
                                src={normalizeDataUrl(leccion.preguntas[0].url_multimedia, 'image')}
                        ...
                        {leccion.tipo_leccion === 'formulario' ? (
                          (() => {
                            // Solo permitir formulario si todas las demás lecciones del módulo están completadas
                            const prerequisitesCompleted = (modulo.lecciones || []).
                              filter((l) => l.tipo_leccion !== 'formulario')
                              .every((l) => l.completada);

                            if (leccion.completada) {
                              return (
                                <button className={`${styles.btnReproducir} ${styles.btnDisabled}`} disabled>
                            ...
                            if (!prerequisitesCompleted) {
                              return (
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                                  <button className={`${styles.btnReproducir} ${styles.btnDisabled}`} disabled aria-disabled>
                                    Completa las lecciones primero
                                  </button>
                                  <span className={styles.hint}>Completa las lecciones del módulo para habilitar el formulario</span>
                                </div>
                              );
                            }
                            ...
                            return (
                              <button
                                className={styles.btnReproducir}
                                onClick={() => navigate(`/capacitaciones/${capacitacion.id}/formulario/${moduloIndex}/${leccionIndex}`)}
                              >
                                Realizar formulario
                              </button>
                            );
                          ...
                          leccion.tipo_leccion === 'imagen' || leccion.tipo_leccion === 'pdf' ? (
                            <button
                              className={styles.btnReproducir}
                              onClick={() => navigate(`/capacitaciones/${capacitacion.id}/imagen/${moduloIndex}/${leccionIndex}`)}
                            >
                              {leccion.tipo_leccion === 'pdf' ? 'Ver PDF' : 'Ver imagen'}
                            </button>
                          ) : (
                            <button
                              className={styles.btnReproducir}
                              onClick={() => navigate(`/capacitaciones/${capacitacion.id}/reproducir/${moduloIndex}/${leccionIndex}`)}
                            >
                              Reproducir
                            </button>
                          )
